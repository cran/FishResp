...
#'
#' Graphical quality control tests for animal activity in
#' chambers over the period of measurements defined in the
#' function \code{\link{correct.meas}}. The function is
#' used for determination of time period for calculation
#' of standard or resting metabolic rate. Note, that
#' mass-specific metabolic rate is calculated for each
#' period of measurements (not raw data).
#'
#' @param clean.data  a data frame obtained by using the function
#' \code{\link{correct.meas}}
#' @param compare  logical: if TRUE then two graphs are plotted to
#' compare mass-specific metabolic rate before and after correction
#' for background respiration
#'
#' @usage
#' QC.activity(clean.data, compare = TRUE)
#'
#' @details
#' QC.activity uses functions \code{\link{extract.slope}}
#' and \code{\link{calculate.MR}} with default parameters
#' (excluding r2 = 0) to plot a graph of animal activity
#'
...
QC.activity <- function(clean.data, compare = TRUE){
  Chamber.No<-NULL
  slope.data.all<-extract.slope(clean.data, r2=0, n.slope=1000, method = "min", plot.data = FALSE)
  ...
  if(compare == TRUE){
    ...
    xyplot(MR.mass + MR.mass.with.BR ~ Date.Time, groups=Chamber.No, grid = TRUE,
           ...
           xlab="Time", ylab="Mass-specific metabolic rate (mgO2/kg/h)",
